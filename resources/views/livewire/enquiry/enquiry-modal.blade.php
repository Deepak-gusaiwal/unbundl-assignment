<div  id="enquiryModal" class="{{$isOpenModal ? "flex" :"hidden"}} justify-center  bg-black/80 fixed inset-0 p-2">
    <div class="bg-white max-w-[600px] w-full mx-auto p-4 rounded-md shadow-md relative isolate overflow-y-auto">
        ...
        <div class="my-2 flex justify-center items-center gap-2">
            @if ($car && $car->thumbnailImage)
              <div class="imgBox w-[200px] aspect-square" >
                  <img class="w-full h-full object-contain" src="{{ $car->thumbnailImage->getImageUrl() }}">
              </div>
              <h2 class="text-center capitalize font-semibold text-xl">{{ $car->title }}</h2>
            @endif
        </div>
