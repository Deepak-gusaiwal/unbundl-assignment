...
>
    <div class="dz-flex dz-flex-col dz-items-start dz-h-full dz-w-full dz-justify-center dz-bg-white p-2">
        @if(! is_null($error))
        ...
        <div class="dz-flex dz-justify-between dz-w-full dz-mt-2">
            ...
            <div class="block text-center" x-show="isLoading" role="status">
        ...
        <div class="dz-flex dz-flex-wrap dz-gap-x-10 dz-gap-y-2 dz-justify-start dz-w-full dz-mt-5 grid md:grid-cols-5  sm:grid-cols-2 grid-cols-1 gap-2 max-h-[200px] overflow-y-auto">
          <div class="col-span-full p-2 flex items-center gap-2 sticky top-0 bg-white">
          <p class=" text-secondary-600 font-bold capitalize p-2 bg-secondary-50 rounded-md w-fit shadow-md">files : {{count($files)}}
            </p>
            @if (count($files) > $max_no_file)
            <p class=" text-red-600 capitalize font-semibold ">maximum allowed files are 10</p>
            @endif
          </div>
            @foreach($files as $file)
                <div class="dz-flex dz-items-center dz-justify-between dz-gap-2 dz-border dz-rounded dz-border-gray-200 dz-w-full dz-h-auto dz-overflow-hidden">
                    ...
                    <div class="dz-flex dz-items-center dz-mr-3 p-1 bg-red-600 hover:bg-red-700 text-white rounded-md shadow-md cursor-pointer">
                        <button type="button" @click="removeUpload('{{ $file['tmpFilename'] }}')">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="white" class="dz-w-6 dz-h-6 dz-text-black">
